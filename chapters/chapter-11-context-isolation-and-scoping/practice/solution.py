"""
Chapter 11 Practice Bank: Context Isolation and Scoping -- REFERENCE SOLUTION
...
scenario_2_naive_tokens = NORWICK_CASE_FACTS_TOKENS + NORWICK_PRIOR_OPINION_RAW_TOKENS
scenario_2_naive_overflows = scenario_2_naive_tokens > NORWICK_BUDGET
scenario_2_scoped_tokens = NORWICK_CASE_FACTS_TOKENS + NORWICK_SHARED_GUIDELINE_TOKENS
scenario_2_scoped_fits = scenario_2_scoped_tokens <= NORWICK_BUDGET
...
scenario_4_handoff_fields = {"vessel_id", "safety_code_version_applied", "inspection_date"}
...
scenario_6_answers = {k: _classify_vantree(v) for k, v in VANTREE_PAYLOADS.items()}
...
scenario_7_fits_per_step = {s: KESTERLY_STEP_SCOPED_TOKENS[s] <= KESTERLY_STEP_BUDGETS[s] for s in KESTERLY_STEP_BUDGETS}
scenario_7_pipeline_total_scoped = sum(KESTERLY_STEP_SCOPED_TOKENS.values())
scenario_7_pipeline_total_budget = sum(KESTERLY_STEP_BUDGETS.values())
...
def score():
    ...
    results.append(("Scenario 1 (Brightfen Regional Utility Outage Response Cooperative, judgment)", scenario_1_answer.strip().lower() == "no", 1))
    ...
    results.append(("Scenario 3 (Marrenfield Regional Crop Insurance Claims Bureau, judgment)", scenario_3_answer.strip().lower() == "no", 1))

    results.append(("Scenario 4 (Coalport Regional Ferry Safety Inspection Authority)", scenario_4_handoff_fields == COALPORT_SAFE_TO_HANDOFF_FIELDS, 1))

    results.append(("Scenario 5 (Sallowbrook Regional Land Trust Conservation Board, judgment)", scenario_5_answer.strip().lower() == "no", 1))
    ...
    results.append(("Scenario 8 (Wolvercote Regional Peer Review Grant Panel, judgment)", scenario_8_answer.strip().lower() == "no", 1))

    return results


def main():
    print("Chapter 11 Practice Bank -- Score Report (reference solution)")
    print("=" * 60)
    results = score()
    total_correct = 0
    total_possible = 0
