"""
Chapter 7 Practice Bank: Multi-Source Context Assembly -- REFERENCE SOLUTION
...
scenario_4_answers = dict(WEXFORD_CLAIM_PAIRS)
...
scenario_6_kept_source = _highest_authority_instance(CINDERFIELD_INSTANCES)[0]
scenario_6_tokens_saved = sum(t for _, t in CINDERFIELD_INSTANCES) - _highest_authority_instance(CINDERFIELD_INSTANCES)[1]
...
scenario_7_naive_contains_contradiction = True
scenario_7_recipe_resolves_contradiction = True
scenario_7_resolved_total = BARLEYCROFT_NAIVE_TOTAL_TOKENS - BARLEYCROFT_TOKENS_FREED
scenario_7_within_budget = scenario_7_resolved_total <= BARLEYCROFT_BUDGET_TOKENS
...
scenario_8_answer = "resolve_or_escalate"
...
def score():
    ...
    results.append(("Scenario 1 (Juniper Ridge Veterinary Partners, judgment)", scenario_1_answer.strip().lower() == "no", 1))
    ...
    results.append(("Scenario 3 (Tamworth Regional Housing Trust, judgment)", scenario_3_answer.strip().lower() == "no", 1))
    ...
    results.append(("Scenario 5 (Dovetail Woodcraft Guild, judgment)", scenario_5_answer.strip().lower() == "no", 1))
    ...
    results.append(("Scenario 8 (Yewmarsh Wildlife Sanctuary, judgment)", scenario_8_answer.strip().lower() == "resolve_or_escalate", 1))

    return results


def main():
    print("Chapter 7 Practice Bank -- Score Report (reference solution)")
    print("=" * 60)
    results = score()
    total_correct = 0
    total_possible = 0
