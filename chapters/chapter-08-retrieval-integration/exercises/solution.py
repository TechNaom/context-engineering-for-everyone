"""
Chapter 8 Exercises: Retrieval Integration -- REFERENCE SOLUTION
...
# ===========================================================================
# Exercise 3 (production-gear) -- relevance-floor filtering.
# ===========================================================================
...
exercise_3_survivors = {
    key for key, score in EXERCISE_3_CHUNKS.items() if score >= RELEVANCE_FLOOR
}
...
exercise_4_kept_chunks, exercise_4_total_tokens = _boundary_safe_fit(
    EXERCISE_4_SURVIVORS_WITH_TOKENS, DOSSIERLINE_BUDGET_TOKENS
)
...
exercise_5_answers = {
    key: _is_provenance_complete(meta) for key, meta in EXERCISE_5_CHUNK_METADATA.items()
}
...
# sequence positions are consecutive.
# ===========================================================================
...
exercise_6_answers = {key: _should_stitch(pair) for key, pair in EXERCISE_6_CHUNK_PAIRS.items()}
...
exercise_7_naive_drops_load_bearing_clause = True
exercise_7_recipe_preserves_load_bearing_clause = "chunk_adjacent_policy_paragraph" in exercise_4_kept_chunks
exercise_7_recipe_within_budget = exercise_4_total_tokens <= DOSSIERLINE_BUDGET_TOKENS
...
def main():
    ...
    total_correct = 0
    total_possible = 0
    print("Chapter 8 Exercises -- Score Report (REFERENCE SOLUTION)")
    print("=" * 60)
    ...
    print(
        f"Boundary-safe fit kept: {exercise_4_kept_chunks} "
        f"| total tokens: {exercise_4_total_tokens}/{DOSSIERLINE_BUDGET_TOKENS}"
    )
