...
# TODO-equivalent (already filled in for the reference solution):
scenario_2_survived = [f for f in LOAD_BEARING if f in COMPRESSED_SUMMARY]
scenario_2_fidelity_ok = len(scenario_2_survived) == len(LOAD_BEARING)
...
scenario_3_winner = min(CFL_40218_CLAIMS, key=lambda c: AUTHORITY[c["source"]])["value"]
...
scenario_4_curated = {k: RAW_WEATHER_RESULT[k] for k in NEEDED_FIELDS}
scenario_4_curated_tokens = sum(len(str(v).split()) for v in scenario_4_curated.values())
...
def main():
    ...
    total_correct = 0
    total_possible = 0
    print("Chapter 13 Practice Bank -- Score Report (REFERENCE SOLUTION)")
    print("=" * 60)
