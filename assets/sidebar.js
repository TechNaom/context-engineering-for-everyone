/*
  ...
  Mirrors the rest of the TechNaom ecosystem's navigation contract.
*/
...
(function () {
  ...
  function isComplete(chapterId) {
    return !!(window.CEFEProgress && window.CEFEProgress.isComplete(chapterId));
  }

  function buildSidebarHtml(root, activeChapter) {
    const modules = window.CEFE_MODULES || [];
    let html = `<a href="${root}index.html" class="sidebar-brand">Context Engineering for Everyone</a>`;
    html += `<span class="sidebar-subtitle">What actually goes into the context window, and why</span>`;
