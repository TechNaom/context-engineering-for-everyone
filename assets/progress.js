/*
  Client-side progress tracker for Context Engineering for Everyone.
  Same contract as the rest of the TechNaom ecosystem: no backend, no
  accounts, localStorage only. A chapter is complete when its quiz
  reaches a perfect score.
*/

window.CEFEProgress = (function () {
  const STORAGE_KEY = "cefe-progress-v1";
...
document.addEventListener("DOMContentLoaded", function () {
  if (window.CEFEProgress) window.CEFEProgress.renderBadges();
});
